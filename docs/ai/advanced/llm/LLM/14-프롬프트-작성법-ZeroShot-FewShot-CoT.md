...
> [← 전체 목차](../LLM.md) · 이전: [접두사 캐시와 시맨틱 캐시의 차이](13-접두사-캐시와-시맨틱-캐시.md) · 다음: [프로덕션급 Prompt는 다섯 요소로 조립한다](15-프롬프트-5요소-구조.md)
...
모델이 최종 결과를 서둘러 찍지 않고 사람처럼 단계를 밟게 된다. **강제로 스스로 계단을 놓게 만들어** "한 걸음 틀리면 계속 틀리는" 국면을 뒤집는다.
...
2. **Few-shot이 가르치는 것은 형식이지 능력이 아니다** — 출력 형식과 어조를 매우 빠르게 정렬시키지만 "계산이 맞는가"에는 거의 기여하지 않는다. **Few-shot을 추론 강화 수단으로 여기는 것은 흔한 오용이다. Few-shot은 형식 정렬 도구일 뿐이다.**
