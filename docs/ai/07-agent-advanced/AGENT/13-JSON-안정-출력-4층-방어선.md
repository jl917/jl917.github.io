...
2. **모델의 「예의」가 포맷 킬러다** — RLHF로 훈련된 대화 습관(접두 인사말, 접미 설명, 마크다운 코드 펜스)이야말로 구조화 출력의 최대 오염원이다. 「정렬 목표」와 「공학 목표」가 충돌하는 전형적 사례다.
...
7. **다층 방어는 이식 가능한 공학 패러다임이다** — 「소프트 유도 → 하드 제약 → 밑바탕 차단 → 사후 폴백」이라는 계층 사고는 JSON뿐 아니라 도구 호출 파라미터, 콘텐츠 안전, 길이 통제 등 「모델을 말 듣게 하는」 모든 장면에 똑같이 적용된다.
