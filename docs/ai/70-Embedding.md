...
머신러닝에서 Embedding이란 무엇인가 — 단어·문장·이미지처럼 사람이 다루는 데이터를 의미 관계가 보존된 고차원 벡터로 바꾸는 것을 말합니다.
...
문장을 **임베딩(embedding)**으로 바꿔서 의미적으로 유사한 문서를 찾습니다.

## Vector Database(벡터DB)

### Sparse(희소) Vector 와 Dense(밀집) Vector

글을 벡터DB에 저장할 때는 글 전체를 임베딩할 수도 있고, 주요 키워드만 뽑아 임베딩할 수도 있습니다. 키워드만 뽑아 변환하면 벡터가 표현할 수 있는 차원 중 일부만 사용하게 되고 나머지 많은 차원은 0으로 '비어' 있게 되는데, 이를 희소 벡터라고 합니다. 반면 글 전체를 임베딩 벡터로 변환하면 거의 모든 차원을 사용하게 되며, 이를 밀집 벡터라고 합니다.

희소 벡터는 특정 키워드와 일치하는 문서를 찾는 데 적합하고, 밀집 벡터는 콘텐츠 전체의 의미를 검색하는 데 적합합니다. 희소 벡터는 차원 값 대부분이 0이라 압축하기 쉬워 메모리 효율이 좋다는 장점이 있고, 밀집 벡터는 모든 차원에 값이 들어 있어 의미를 담아내기 좋다는 장점이 있습니다. 다만 희소 벡터는 키워드만으로 완전한 의미를 파악하기 어렵고, 밀집 벡터는 모든 차원의 값을 저장하므로 메모리 사용량이 크다는 단점이 있습니다. 실무에서는 희소 벡터를 BM25 같은 키워드 검색에, 밀집 벡터를 코사인 유사도 기반의 의미 검색에 주로 활용합니다.
