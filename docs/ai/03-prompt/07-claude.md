...
기존 프로젝트를 분석해서 `CLAUDE.md` 를 만들어내는 프롬프트입니다. 아래 전문을 그대로 붙여 넣으면 됩니다. 규칙 파일을 **어떻게 써야 하는지**(무엇이 실제로 지켜지는지)는 [Rule](./05-rule)에 정리해 두었습니다.

> 15개 항목을 한 번에 요구하므로 결과가 깁니다. 마지막 지시대로 `docs/` 아래에 섹션별로 나눠 저장하고, `CLAUDE.md` 에는 요약과 링크만 남기는 편이 실용적입니다. 규칙 파일은 매 요청마다 토큰을 먹기 때문입니다.

## 프롬프트 전문

```text
프로젝트를 분석하고 CLAUDE.md를 작성해주세요.
아래 15가지 핵심 항목을 분석한 뒤, 이를 기반으로 한국어로 문서를 작성해주세요.
docs 폴더에 각 섹션별로 md 파일을 생성해서 정리해주세요.
```
