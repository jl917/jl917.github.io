...
대모델은 System prompt, 개발자 요구, 사용자 질문, 웹페이지 내용, 문서 조각을 전부 하나의 Token 열로 이어붙여 같은 컨텍스트에 밀어 넣는다. 모델은 이 텍스트 위에서 계속 예측할 뿐이며 「어느 문장의 권한이 가장 높은지」 검사하지 않는다.
...
1. **「입력 필터링」이 부족한 이유는 필터링의 전제가 대모델에서는 성립하지 않기 때문이다** — SQL 인젝션은 파라미터화 쿼리로 근절할 수 있는데, 이는 DB 파서가 태생적으로 「명령」과 「파라미터」를 구별하기 때문이다. 대모델의 디코더 앞에는 Token 열 하나만 있을 뿐, 어떤 위치에도 권한 표시가 없다. 방어 조치는 키워드 블랙리스트를 갈아 끼우는 게 아니라 계층 자체를 바꿔야 한다.
