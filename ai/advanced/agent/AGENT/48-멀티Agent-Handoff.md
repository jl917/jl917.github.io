...
> [← 전체 목차](../AGENT.md) · 이전: [사용자가 중간에 딴 질문을 끼워넣으면 Agent는 왜 주작업을 놓치는가](47-작업이탈방지-강약일관성-명령우선순위.md) · 다음: [다중 Agent를 "역할 나눠 대화"로 조율하면 왜 무너지는가](49-다중Agent-중앙조정-상태머신-기억풀.md)
...
1. **Handoff의 본질은 권한 재인가이지 데이터 운반이 아니다** — 인계 시 진짜로 넘겨지는 것은 "누가 무엇을 볼 수 있고 누가 무엇을 할 수 있는가"이며 Token 비용은 부산물일 뿐이다. handoff를 이력 복사로 여기면 보안 모델까지 함께 복사해 망가뜨리는 셈이다.
2. **컨텍스트는 완전할수록 좋은 것이 아니라 "충분하면서 깨끗할수록" 좋다** — 전면 상속은 업스트림의 잘못된 가정과 실패 경로까지 함께 상속시켜, Agent A가 이미 잘못 든 길을 Agent B가 그대로 따라가게 만든다. 정보 완비성과 판단의 순수성은 한 쌍의 모순이다.
3. **"요약해서 Token 절약"은 표층 인식이고 "경계 거버넌스"가 아키텍트 관점이다** — 이 두 답변의 분수령은 권한 필터링, 사실 확인, 실패 기록, 수신자 책임을 언급했는가다.
