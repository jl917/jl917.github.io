...
#### 그런데 자식이 천만 건이고 부모도 크다면?

방금 5% 라는 숫자만 보고 "FK 는 어차피 싸다"라고 외우면 위험합니다. **이 5% 는 "부모가 30행이라 100% 캐시된다"는 특수 조건의 결과**입니다. 진짜 변수는 **자식 행 수가 아니라 "부모 PK 조회가 메모리에서 끝나는가"** 입니다.

> ⚠️ 아래 표는 이 데이터셋에서 잰 값이 아니라 **원리로 추론한 경향**입니다. 실제 배수는 하드웨어·`innodb_buffer_pool_size`·부모 크기·동시성에 따라 달라집니다. 외울 것은 숫자가 아니라 **"무엇이 비용을 결정하는가"** 입니다.

INSERT 한 건이 FK 때문에 추가로 하는 일은 **"부모 인덱스에서 참조값을 찾아 그 행에 공유락(S락)을 거는 것"** 하나입니다. 이 비용은 자식이 20만 건이든 천만 건이든 **한 행당 비용은 동일**합니다. 자식이 50배 늘면 총비용도 그냥 50배 늘 뿐, 행당 오버헤드 비율(5%)은 변하지 않습니다 — **부모 조회가 계속 캐시에서 끝나는 한.**

문제는 부모가 커져서 **버퍼 풀에 다 안 올라올 때** 생깁니다.

| 시나리오 | 부모 PK 조회 | FK 의 INSERT 오버헤드 |
|---|---|---|
| 부모 30행 (실측 1) | 100% 메모리 히트 | **약 5%** — 무시 가능 |
| 부모 수만~수십만 행, 통째로 캐시 | 거의 메모리 히트 | 여전히 한 자릿수 % |
| 부모 수천만 행 + 버퍼 풀 부족 | 조회마다 **디스크 랜덤 I/O** 가능 | **수십~수백 %**, 심하면 몇 배 |
| 위 + 부모 참조값이 랜덤 분포 | 캐시 워밍이 안 됨 (콜드) | 최악 — I/O 병목으로 처짐 |

**즉 "자식 천만 건 적재"가 느려지는 원인은 FK 자체가 아니라, ①부모가 버퍼 풀 밖에 있어 조회가 디스크로 떨어지는 것, ②천만 번의 커밋/언두/보조 인덱스 유지 비용입니다.** 이 둘을 분리해서 봐야 합니다.

- **부모가 작으면 (코드·카테고리·상태 테이블):** 자식이 천만 건이어도 FK 비용은 여전히 저렴합니다. 부모 전체가 메모리에 상주하기 때문입니다. — *대부분의 실무 관계가 여기 해당.*
- **부모도 대형이면 (예: 주문상세 천만 건이 주문 수백만 건을 참조):** 부모의 "핫하지 않은" 부분을 조회할 때마다 디스크를 때리면서 적재 속도가 급락할 수 있습니다.

> 💡 **실무 팁 — 천만 건대 적재를 빠르게**
> 1. **적재 순서를 부모 PK 순으로 정렬**해서 넣으면, 부모 인덱스를 순차적으로 훑어 캐시 히트율이 올라갑니다 (랜덤 접근 → 순차 접근).
> 2. 한 트랜잭션에 천만 건을 다 넣지 말고 **수천~수만 건 단위 배치 커밋**으로 쪼갭니다. 언두 로그·락 보유 시간·복제 지연이 폭증하는 것을 막습니다.
> 3. 검증된 정합성의 대량 마이그레이션이라면 `SET FOREIGN_KEY_CHECKS=0` 으로 적재 후, 이 절 뒤의 [**고아 행 검증 쿼리**](#fk-와-락)를 반드시 돌립니다 (끄고 넣으면 MySQL 은 재검증하지 않습니다).
> 4. 대량 적재 전 보조 인덱스를 잠시 떼고 적재 후 다시 붙이는 방법도 있지만, **FK 컬럼의 인덱스는 FK 가 요구하므로 함부로 못 뗍니다** — FK 를 먼저 끄거나 유지한 채 진행해야 합니다.

> ⚠️ **함정 — 동시 적재에서는 부모 S락이 진짜 병목**
> 단일 세션 적재는 위처럼 캐시 문제지만, **여러 세션이 동시에 천만 건을 나눠 넣으면** 이야기가 달라집니다. 인기 있는 부모 행(예: "기본 카테고리", "게스트 사용자") 하나를 여러 INSERT 가 동시에 참조하면, 그 행의 S락을 두고 경합이 생깁니다. S락끼리는 호환되지만, 누군가 그 부모를 `UPDATE`(X락) 하려는 순간 전체가 줄줄이 대기합니다 (아래 [FK 와 락](#fk-와-락) 참조). 자식 규모가 클수록 이 hot row 문제가 두드러집니다.
...
> 📎 **더 깊이 — [부록 B: 외래키(FK) 실무 완전 정복](../appendix-b-foreign-keys/index.md)**
> 실무에서 FK 로 실제로 부딪히는 것들을 원인·재현·해결까지 모았습니다:
> FK 에러 코드 카탈로그(`1452`/`1451`/`3730`/`3780`/`errno 150`), 타입·콜레이션 불일치로 FK 가 안 걸리는 문제,
> 생성·삭제·적재 **순서** 및 순환 참조, "애플리케이션 레벨 FK"의 트레이드오프, ORM(JPA)·`gh-ost`·파티셔닝의 함정,
> FK 가 만드는 **데드락**과 hot row 락, **고아 행** 찾기/정리, `TRUNCATE`·인덱스 삭제 제약.
...
> 📎 **더 깊이 — [부록 C: 정규화 완전 정복 (1NF~BCNF)](../appendix-c-normalization/index.md)**
> 함수 종속·이상현상의 원리부터, 하나의 예제를 **0NF→1NF→2NF→3NF→BCNF** 로 직접 분해하는 손기술, 4NF/5NF 개관,
> 실무 위반 패턴 카탈로그(콤마 문자열·EAV·계산 컬럼), 반정규화 동기화 검증 쿼리까지 모았습니다.
