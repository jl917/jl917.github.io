...
> ⚠️ **함정**: `WITH ROLLUP` 과 `ORDER BY` 를 같이 쓰면 총계 행의 위치가 의도와 다를 수 있습니다(NULL 정렬 규칙 때문). 또 `WITH ROLLUP` 이 붙은 쿼리에는 `DISTINCT` 나 `LIMIT` 을 조합할 때 주의가 필요합니다. 복잡해지면 애플리케이션에서 소계를 계산하거나, 윈도우 함수(Step 17)를 쓰는 편이 깔끔할 때가 많습니다.
...
4. `COUNT(*)` 와 `COUNT(phone)` 이 다른 값이 나오는지 확인하세요. 왜 그런가요? (customers 테이블)
...
- **[문제 4]** 는 `COUNT(*)` 와 `COUNT(phone)` 이 왜 30 과 27 로 갈리는지 **주석으로 설명**까지 요구합니다. 숫자를 맞히는 게 아니라 이유를 말로 쓰는 게 핵심입니다. `birth_date` 도 NULL 이 섞인 컬럼이므로 같은 원리를 확인할 수 있습니다.
